...
class UserProfileHeader: UICollectionViewCell {
    ...
    let userInfo: AuthorizedUserInfo = FirebaseService()
    let followingProvider: FollowingProvider = FirebaseService()
    ...
    let gridButton: UIButton = {
        ...
        button.setImage(UIImage.assetImage(AssetImagesNames.grid), for: .normal)
        return button
    }()
    
    let listButton: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage.assetImage(AssetImagesNames.list), for: .normal)
    ...
    let bookmarkButton: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage.assetImage(AssetImagesNames.ribbon), for: .normal)
        button.tintColor = UIColor(white: 0, alpha: 0.2)
        return button
    }()
    ...
    fileprivate func configureEditProfileButtonView(as userStatus: UserStatus) {
        switch userStatus {
        case .current:
...
extension UserProfileHeader: HeaderDelegate {
    func setUserWithProfileImage(user userWithProfileImage: UserWithProfileImage) {
        self.userWithProfileImage = userWithProfileImage
        usernameLabel.text = userWithProfileImage.user.name
        profileImageView.image = userWithProfileImage.profileImage
    }
    ...
    func setUserStatus(userStatus: UserStatus) {
        ...
        configureEditProfileButtonView(as: userStatus)
    }
    
    func clearHeader() {
        profileImageView.image = nil
        usernameLabel.text = ""
        configureEditProfileButtonView(as: .following)
    }
}
